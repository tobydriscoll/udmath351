...
Here is **how NOT to multiply two matrices**:

$$
\twomat{1}{2}{-2}{0} \twomat{3}{-4}{-1}{2} = \twomat{1\cdot 3}{2\cdot(-4)}{(-2)\cdot (-1)}{0\cdot 2}. 
$$

Technically, the above is called a *Hadamard product*, and it actually does come in handy sometimes. But it is less than 1/1000th as important as a different, less-obvious definition of multiplication that is tied to deep mathematical structure.

### Matrix times vector

The idea of linear combinations, as defined in @def-nullspace-linear-comb, serves as the foundation of multiplication between a matrix and a vector.
...
:::{.solution}
All we need to do is to check that the produc, in either order, is the identity matrix:
...
The solution formula $\bfx=\bfA^{-1}\bfb$ from the proof of @thm-FTLA1 is theoretically valuable but impossible to apply to specific system without knowing how to calculate the matrix inverse. In general, doing so is harder than just doing row elimination on a linear system, so it's not a useful algorithm. 

There is a method for computing the inverse based on row elimination. But there is little cause for using it in practice. We will just look at some particular cases where the inverse is very easy to find.
...
We encountered the null space of a matrix in @sec-linsys-nullspace. It's a special case of a vital type of set.
...
Span and independence are two great tastes that taste great together.
...
::::{#def-linalg-eigen} Eigenvalue and eigenvector
...
where (as defined in @eq-diagonalization-VD) $\mathbf{D}$ is a diagonal matix of eigenvalues and $\bfV$ is a square matrix whose columns are corresponding eigenvectors.
::::
