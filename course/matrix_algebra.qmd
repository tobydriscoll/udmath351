...
1. The RRE form of $\bfA$ is the identity matrix.
2. $\rank(\bfA)=n$.
::::

:::{.callout-important}
Because the statements in @thm-FTLA1 are equivalent, so are their negations. That is, if $\bfA$ is an $n\times n$ matrix, then these statements are all equivalent as well:

1. $\bfA$ is singular.
2. The linear system $\bfA\bfx=\bfb$ has infinitely many solutions or is inconsistent.
1. The RRE form of $\bfA$ has at least one row of zeros.
2. $\rank(\bfA) < n$.
:::

:::{.callout-note}
The solution formula $\bfx=\bfA^{-1}\bfb$ from @thm-FTLA1 is theoretically valuable but can be applied only if the inverse is available. Computing a matrix inverse is usually harder than doing row elimination on a linear system, so it's not a useful algorithm in general. 
:::
...
2. Every linear combination of vectors in $S$ is also in $S$. (This is called *closure under linear combination*.)
::::
...
The equation $x + 2y + 3z = 0$ describes a plane passing through the origin in $\real^3$. It's clear geometrically that scaling a vector in the plane leaves you in the plane, and adding vectors in the plane does as well. This is enough to show that this plane is a subspace of $\real^3$.

 <!-- In fact, it is the null space of the matrix $\begin{bmatrix} 1 & 2 & 3 \end{bmatrix}.$ -->

The equation $x+y+z=1$ is also a plane in $\real^3$, but it does not pass through the origin, so it cannot be a subspace.
::::

:::{#exm-subspaces-zero chapter=4 description="The trivial subspace"}
The singleton set $\{\bfzero\}$ is a subspace of $\real^n$ for any $n$. It's called the *trivial subspace*.
:::
...
::::{#exm-subspaces-column chapter=4 description="Is it in the column space?"}
Is $\threevec{1}{2}{3}$ in the column space of $\bfA = \begin{bmatrix} 1 & 1 \\ -1 & 1 \\ 2 & 1 \end{bmatrix}$?

:::{.solution}
We can check by looking at the consistency of the system $\bfA\bfx = \threevec{1}{2}{3}$. The augmented matrix is

$$
\augmat{\begin{matrix} 1 & 1 \\ -1 & 1 \\ 2 & 1 \end{matrix}}{\begin{matrix} 1 \\ 2 \\ 3 \end{matrix}} 
\stackrel{\text{RRE}}{\longrightarrow}
...
::::{#exm-nullspace-span chapter=3 description="Expressing nullspace via span"}
...
:::{.solution}
If we use the given vectors as columns of a matrix, then their span is equivalent to the column space of the matrix:
...
# Dimension
The **dimension** of a subspace $S$, written $\dim(S)$, is the number of vectors in any basis of $S$. The dimension of the singleton subspace $\{\bfzero\}$ is defined to be zero.
::::
...
::::{#exm-subspaces-dimension chapter=4 description="Too few to span"}
Determine whether the vectors $\bfv_1=[1,2,3]$, $\bfv_2=[2,4,0]$ are a basis of $\real^3$.

:::{.solution}
Since $\real^3$ is 3-dimensional, and there are only 2 vectors, it's impossible for them to span it, so they are not a basis.
:::
::::

::::{#exm-subspaces-dimension chapter=4 description="Too many to be independent"}
Determine whether the vectors $\bfv_1=[1,2]$, $\bfv_2=[3,4]$, $\bfv_3=[1,1]$ are a basis of $\real^2$.

:::{.solution}
Since $\real^2$ is 2-dimensional, and there are 3 vectors, it's impossible for them to be independent, so they are not a basis.
:::
::::
...
:::{.solution}
We find a basis for their span by putting them as columns of a matrix, then looking at its column space:
...
|:------------------:|:---------:|:------------:|
| $\bfb$ is a combination of columns | $\bfb\in \colsp(\bfA)$; $\bfA \bfx = \bfb$ | Solve $\augmat{\bfA}{\bfb}$ | 
...
We collect these properties here for reference. Except for the first one, they are all easy to prove.

::::{#thm-eigenvalues-properties}
Suppose $\bfA$ is an $n\times n$ square matrix.
...
Given an $n\times n$ matrix $\bfA$, the following procedure can be used to find its eigenvalues and eigenspaces:
...
::::{#exm-eigenvalues-triangular chapter=4 description="Complex eigenvalues"} 
...
To find a basis we just use the first row as explained in @thm-eigenvalues-2by2, getting $\twovec{1}{i}$. 

Since the matrix is real, we can apply @thm-eigenvalues-conjugate. A basis for the other eigenspace can be found by conjugating this one, to get $\twovec{1}{-i}$.
:::
::::

## Diagonalization 

The eigenvalues of a matrix are the roots of its characteristic polynomial. In the general $n\times n$ case, we can factor it as

$$
p(z) = (z-\lambda_1)^{m_1}(z-\lambda_2)^{m_2}\cdots(z-\lambda_k)^{m_k},
$$ {#eq-charpoly-factor}

for positive integer multiplicities such that $m_1+\cdots+m_k=n$. The easiest situation is when all of the exponents are 1, and we say every eigenvalue is **simple**. 

Explaining what happens when at least one eigenvalue is not simple takes some more effort.

### Multiplicity

The exponents in @eq-charpoly-factor are one of *two* ways to define multiplicities of the eigenvalues.

::::{#def-linalg-algmult}
# Algebraic multiplicity
The **algebraic multiplicity** (AM) of an eigenvalue is its multiplicity as a root of the characteristic polynomial.
::::

The following example illustrates a peculiar possibility for eigenvalues of AM greater than 1.

::::{#exm-diagonalization-defective chapter=4 description="Defective eigenvalue"}
...
This leads us to define the second notion of multiplicity for an eigenvalue. 

::::{#def-linalg-geomult}
# Geometric multiplicity
The **geometric multiplicity** (GM) of an eigenvalue is the dimension of its associated eigenspace.
::::
...
The distinction between these cases is significant enough to warrant yet another definition.
...
As we will see later, defective matrices complicate the application of eigenvalue analysis. 

Since $1\le \text{GM} \le \text{AM}$, there is a simple and common case in which we are certain that a matrix is not defective.

::::{#thm-}
# Distinct eigenvalues
If $\bfA\in\cmn{n}{n}$ has $n$ distinct eigenvalues, i. e., if all the eigenvalues are simple, then $\bfA$ is not defective.
::::

For $n=2$, the alternative possibilities are still pretty easy to lock down. 
...
In essence, @thm-la-2x2defective says that @exm-diagonalization-defective and @exm-diagonalization-nondefective cover all the possibilities for $2\times 2$ matrices.
...
where $\mathbf{D}$ is a diagonal matrix of eigenvalues and $\bfV$ is a square matrix whose columns are corresponding eigenvectors, as defined in @eq-diagonalization-VD.
::::
