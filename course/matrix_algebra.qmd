...
We can think the same way about multiplying a matrix or vector by a number:
...
While we're on a roll, you would probably expect that we define matrix multiplication the same way:
...
But we don't! OK, technically this is called a Hadamard product, and it has some uses. But 99.99999% of the time a different, less obvious way of multiplying matrices does a better job of respecting critical mathematical structure. 
...
However, there is a big departure from multiplication as we usually know it.
...
Finally, we observe that if $\bfA$ is $m\times n$ and $\bfx$ is an $n$-vector, then $\bfA\bfx$ gives the same result whether we interpet $\bfx$ as a vector or as an $n\times 1$ matrix.
...
This is clearly impossible for any chocies of $a,b,c,d$. Hence $\bfA$ is singular.
::::
...
Formally for part 3 of the theorem, a matrix $\bfA$ is **upper triangular** if $A_{ij}=0$ whenever $i>j$, and it is **lower triangular** if $A_{ij}=0$ whenever $i<j$.
