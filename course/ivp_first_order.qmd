...
The name means that the problem doesn't depend on any external factors---it is entirely self-contained.
...
Suppose that $\hat{x}$ is a root of the function $f$, that is,  it satisfies $f(\hat{x})=0$. Then the constant function $x(t)\equiv \hat{x}$ a solution of @eq-autonomous-autonomous. 
...
While the mathematical definition of asymptotic stability looks technical, it simply means that if you start close enough to $\hat{x}$, then you go back to it in the limit $t \to \infty$. 

There is a weaker form of the property that is a bit more subtle:

::::{#def-qualitative-stability}
#  Stability
The steady state $\hat{x}$ is said to be **stable** if, for any $\epsilon>0$, you can ensure that $|x(t,X)-\hat{x}|<\epsilon$ for all time whenever $|X-\hat{x}|<\delta$, where $\delta$ depends on $\epsilon$. If there is no such way to choose $\delta$, then $\hat{x}$ is **unstable**.
::::

::: {.callout-important}
If an equilibrium value is asymptotically stable, then it is stable. But the converse is not always true.
:::
...
:::{.callout-warning}
Never overlook the $a_0(t)$ in @eq-linear-def. If you forget to divide through by it to get the standard form, everything that follows will be wrong.
:::
...
Note that because of @thm-linear-super, the homogeneous part $x_h$ will contain the integration constant. We elaborate on steps 1 and 2 in the next two sections.

## Homogeneous equations {#sec-first-order-homogeneous1}

Step one of our solution strategy in @thm-linear-solve is to solve the homogeneous problem $\opA[x]=0$, or
...
:::{.callout-note}
We can ignore the integration constant that arises from finding $q$ in @thm-homogeneous1-solution. It would just end up being absorbed into the $c_1$, whose existence is already known from @cor-linear-super.
:::
...
@sec-first-order-homogeneous1 explained how to find the general solution $x_h(t)$ of a homoegenous linear system $\opA[x] = 0$. The next step of our solution strategy in @thm-linear-solve is to find some particular solution of $\opA[x]=f$ for the given $f(t)$.
...
::: {.callout-warning}
The unstated very first step in @thm-firstlin-varpar is to put the given ODE into the standard form, $x' + P(t) x = f$. On this, everything depends.
:::
