...
:::{.callout-attention}
@eq-structure-wronskian is a *determinant*, which we will not learn in general for a while yet. But for now we only care about the $2\times 2$ case, in which 
...
Part of what makes @eq-homogeneous-distinct work is that $e^{\lambda_1 t}$ and $e^{\lambda_2 t}$ are indepdendent solutions, because
