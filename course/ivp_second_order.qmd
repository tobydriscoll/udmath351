...
::::{#def-second-linear-ODE}
A linear, 2nd-order ODE in standard form is
$$
x'' + p(t)x' + q(t) x = f(t),
$$ {#eq-second-linear}
where $p$, $q$, and $f$ are specified functions of the independent variable.
::::

For a linear initial-value problem, we would also specify two conditions $x(a)=x_0$ and $x'(a)=v_0$, like in @eq-second-ivp.

Equation @eq-second-linear is in the form $\opA[x]=f$ for the linear operator defined by

$$
\opA[x] = \ddd{x}{t} + p \dd{x}{t} + q x.
$$

Much of what we learned using the abstract language of operators we introduced for first-order problems applies here as well. In particular, we still have a superposition theorem like @thm-linear-super and a solving strategy like @thm-linear-solve:
...
:::{#exm-structure-dependent chapter=2 description="Dependent functions"}
The functions $u_1=t$ and $u_2=5t$ are linearly dependent on any interval, because $-5u_1 + u_2 = -5t+5t=0$ for all $t$.
:::

:::{#exm-structure-independent chapter=2 description="Independent functions"}
...
:::{.callout-note}
@eq-structure-wronskian is a *determinant*, which we will not learn about in general for a while yet. In the $2\times 2$ case, it's
...
::::{#exm-structure-wronskian chapter=2 description="Wronskian"} 
...
$$ {#eq-2nd-homogeneous-solution}

where $c_1$ and $c_2$ are arbitrary constants. Furthermore, the general solution of the nonhomogeneous problem $\opA[x]=f$ is

$$
x(t) = x_h(t) + x_p(t),
$$ {#eq-2nd-general-solution}

where $x_h$ is the homogeneous solution @eq-2nd-homogeneous-solution and $x_p$ is any one particular solution of $\opA[x]=f$.
::::
...
::::{#def-second-cc}
The second-order, linear, **constant-coefficient** problem in standard form is 
$$
x'' + bx' + k x = f(t),
$$ {#eq-second-cc-def}
...
::::{#exm-homogeneous-trivial chapter=2 description="Characteristic equation---double root"}
...
::::{#exm-homogeneous-real-distinct chapter=2 description="Characteristic equation---distinct real roots"}
...
::::{#exm-homogeneous-real-ivp chapter=2 description="2nd order IVP"}
...
::::{#exm-complex-fraction chapter=2 description="Complex number fractions"}
$$
...
::::{#exm-complex-reciprocal chapter=2 description="Complex number reciprocal"}
...
::::{#exm-complex-polar chapter=2 description="Complex numbers polar form"}
...
::::{#exm-complex-dummy chapter=2 description="Characteristic polynomial---complex roots"}
Solve the IVP $x''+9x=0$, $x(0)=2$, $x'(0)=-12$.
::::

::::{#exm-complex-simple chapter=2 description="Characteristic polynomial---complex roots"}
Solve the IVP $x''+9x=0$, $x(0)=2$, $x'(0)=-12$.
...
::::{#exm-complex-complex1 chapter=2 description="Characteristic polynomial---complex roots"} 
...
::::{#exm-charpoly-real-solution chapter=2 description="Characteristic polynomial---real solution for complex roots"}
...
::::{#exm-complex-complex2 chapter=2 description="Characteristic polynomial---real IVP for complex roots"} 
Solve the IVP of @exm-complex-complex1 without using complex numbers to express the solution.
...
Let $x_1$ and $x_2$ be independent solutions of the homogeneous equation $\opA[x]=0$, and let $W(x_1,x_2)$ be their Wronskian. For any given forcing function $f$, define

$$
u_1 = \int \frac{-x_2 f}{W(x_1,x_2)}\, dt, \quad u_2 = \int \frac{x_1 f}{W(x_1,x_2)}\, dt.
$$ {#eq-second-vop}
...
:::{.callout-important}
The integration constants for $u_1$ and $u_2$ in @eq-second-vop can be taken to be zero. 
:::

::::{#exm-vop2-cons chapter=2 description="2nd order variation of parameters"} 
Find a particular solution of $x'' - x = 4$. 

:::{.solution}
There's no getting around the need for the homogeneous solution here. The characteristic roots are $\pm 1$, so we take $x_1=e^t$, $x_2=e^{-t}$. Then 
...
::::{#exm-variation_parameters-resonance chapter=2 description="2nd order variation of parameters"} 
...
::::{#exm-undetermined-const chapter=2 description="Undetermined coefficients---constant"} 
Rework @exm-vop2-cons, finding a particular solution of $x'' - x = 4$. 
...
::::{#exm-MUC-polynomial chapter=2 description="Undetermined coefficients---polynomial"}
...
::::{#exm-MUC-exponential chapter=2 description="Undetermined coefficients---exponential"}
...
::::{#exm-MUC-trig  chapter=2 description="Undetermined coefficients---trig"}
...
::::{#exm-undetermined-resonance chapter=2 description="Undetermined coefficients (failure)"}
...
:::{#exm-undetermined-resonance-fix chapter=2 description="Undetermined coefficients---workaround"}
...
::::{#exm-oscillator-frequency chapter=2 description="Natural frequency of an oscillator"}
...
::::{#exm-damping-strength chapter=2 description="Damping strength"}
...
::::{#exm-free-oscillations chapter=2 description="Free oscillations"} 
