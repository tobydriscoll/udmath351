...
Equation @secondlin-linear is in the form of $\opA[x]=f$ for the linear operator
...
We now reap rewards for using the abstract language of operators with first-order problems, because much of what we learned there applies here as well. In particular, we still have a [superposition theorem](theorem-linear-super), and  @Algorithm {number} <algorithm-linear-solve> still provides a blueprint for the solution process:
...
:::{.callout-attention}
Equation @eq-structure-wronskian is a *determinant*, which we will not learn in general for a while yet. But for now we only care about the $2\times 2$ case, in which 
...
:::{.callout-note}
In @second_linear-cc-def the coefficient $b$ has units $1/T$, the coefficient $k$ has units $1/T^2$, and $f(t)$ has units $X/T^2$, where $X$ and $T$ are the units of $x$ and $t$, respectively.
:::
...
::::{#def-} Characteristic polynomial of a linear ODE
...
$$ {#eq-const_coeff-distinct}
::::

Part of what makes @eq-const_coeff-distinct work is that $e^{\lambda_1 t}$ and $e^{\lambda_2 t}$ are indepdendent solutions, because
...
All seems well, until you remember that the roots of a real quadratic polynomial may come as a complex conjugate pair. How then do we interpret @eq-const_coeff-distinct or @second_linear-homogeneous-repeated? The answer is what makes second-order problems really different from first-order ones, and we tackle it by taking a brief deep dive into complex numbers.
...
We call @secondlin-polar the **polar form** of a complex number, because it expresses $z$ as a distance from zero and an angle from the positive Re axis. Just as with any point in the plane, we can express a complex number either in Cartesian form using Re and Im parts, or in polar form using modulus and Euler's identity.
...
Now we can finish the story of computing solutions to homogeneous constant-coefficient ODEs. We can still use @Theorem {number} <theorem-second_linear-homogeneous-distinct> and @Theorem {number} <theorem-second_linear-homogeneous-repeated>, with the complex-valued interpretations of the exponentials.
...
In @Example {number} <example-complex-simple> this would have led us right to $2\alpha=2$ and $3i(2i\beta)=-12$.
...
::::{#exm-} Example
In @Example {number} <example-complex-simple>, the roots were $0 \pm 3i$. Thus another expression for the general solution is
...
::::{#exm-} 
Solve the IVP of @Example {number} <example-complex-complex1> without using complex numbers.
...
Equations @eq-variation_parameters-eq1 and @eq-variation_parameters-eq2 impose
...
where $b$ and $k$ are constants, and $f$ appears in @table-undetermined-forcing. 
...
Corresponding to each $f$ in @table-undetermined-forcing is a specific form for $x_p$ containing one or more constants (given by capital letters) that must be deduced by substituting into the particular ODE under consideration. This procedure is what gives the method its name, the **method of undetermined coefficients**.

(example-undetermined-const)=
::::{#exm-} 
Rework @Example {number} <example-variation_parameters-const>, finding a particular solution of $x'' - x = 4$. 
...
The examples above are the fundamental ones. If you have a forcing function that is a sum of terms from @table-undetermined-forcing, then you can treat each term separately as above and add together the particular solutions found.
...
The failure of @Example {number} <example-undetermined-resonance> at $\omega = 1$ was due to the fact that the $x_p$ we tried is actually a homogeneous solution. This is not too hard to fix, but a general description is not worth the effort; we can always fall back to variation of parameters. 

(example-undetermined-resonance-fix)=
::::{#exm-}
We can make the tricky case $x''+x=\cos(t)$ from @Example {number} <example-undetermined-resonance> quite easy by relating it to complex exponentials. Since $\cos(t)$ is the real part of $e^{it}$, it follows that the solution we seek is the real part of any particular solution of 

$$
x'' + x = e^{it}.
$$

Referring to @Example {number} <example-variation_parameters-resonance> with $\lambda=i$, we use variation of parameters to find the particular solution
...
:::{.callout-note}
Equation @eq-oscillators-free applies equally well to horizontal and vertical oscillations. Gravity does not appear in the vertical case because it is accounted for by measuring $x$ from the static equilibrium position.
:::
...
If the forcing is exactly at the natural frequency, then the situation is like that of @Example {number} <example-variation_parameters-resonance>:
